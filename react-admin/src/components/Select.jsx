...
export default function Select({ value, onChange, options, leadingIcon: LeadingIcon, placeholder = "Select…", className = "" }) {
    ...
    <RSelect.Root value={value} onValueChange={onChange}>
      ...
      >
        <span className="flex items-center gap-2 truncate">
          {LeadingIcon ? <LeadingIcon className="h-4 w-4 text-slate-400" /> : null}
          <RSelect.Value placeholder={placeholder}>{current?.label || placeholder}</RSelect.Value>
        </span>
        <RSelect.Icon>
          <ChevronDown className="h-4 w-4 text-slate-400 transition group-data-[state=open]:rotate-180" />
        </RSelect.Icon>
      </RSelect.Trigger>
      <RSelect.Portal>
        <RSelect.Content
          position="popper"
          sideOffset={6}
          className="z-50 min-w-[var(--radix-select-trigger-width)] overflow-hidden rounded-lg border border-slate-200 bg-white p-1 shadow-xl"
        >
          <RSelect.Viewport>
            {options.map((opt) => (
              <RSelect.Item
              ...
              >
                {opt.icon ? <opt.icon className="h-4 w-4 text-slate-400" /> : null}
