...
const FACEBOOK_API = import.meta.env.VITE_FACEBOOK_ANALYTICS_API;
const FACEBOOK_SECRET_KEY = import.meta.env.VITE_FACEBOOK_SECRET_KEY;
const FACEBOOK_USER_ID = import.meta.env.VITE_FACEBOOK_USER_ID;
const MEDIA_URL = import.meta.env.VITE_NAS_BASE_URL;
const facebookAdDetails = async (ad_id) => {
  const payload = {
    ad_id,
  ...
  try {
    const { status, data } = await axios.post(`${FACEBOOK_API}/getAdDetails`, payload);
    if (status === 200) {
