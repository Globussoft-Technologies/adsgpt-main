...
const GOOGLE_API = import.meta.env.VITE_GOOGLE_ANALYTICS_API;
const GOOGLE_SECRET_KEY = import.meta.env.VITE_GOOGLE_SECRET_KEY;
const GOOGLE_USER_ID = import.meta.env.VITE_GOOGLE_USER_ID;
const MEDIA_URL = import.meta.env.VITE_NAS_BASE_URL;

const googleAdDetails = async (ad_id) => {
  const payload = {
    ad_id,
  ...
  try {
    const { status, data } = await axios.post(`${GOOGLE_API}/api/getAdDetails`, payload);
    if (status === 200) {
    ...
    console.error('Error fetching Google ad details:', error);
  }
};
