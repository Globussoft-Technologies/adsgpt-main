...
const ChipDropdown = ({
  ...
  const renderOption = (opt) => {
      ...
      return (
        ...
        >
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              onSelect(isSelected ? '' : opt.voice_id, opt);
            }}
            className="flex min-w-0 flex-1 items-center gap-2 text-left"
          >
            <span
    ...
    // gender / accent / age — plain strings. Clicking an already-selected item toggles it off.
    const isSelected = value === opt;
    return renderSimpleRow(opt, prettify(opt), isSelected, () => onSelect(isSelected ? '' : opt));
  };
