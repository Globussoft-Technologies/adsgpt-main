...
const AdCreativesCard = ({
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="card_header absolute top-0 left-0 z-20 flex h-10 w-full items-center justify-between rounded-none px-3 opacity-100 transition-opacity duration-300 group-hover:opacity-100 lg:opacity-0 2xl:h-12 dark:bg-[#0F0F0F]/80">
        ...
        <div className="right_header flex items-center gap-2 2xl:gap-2.5">
          {/* <button className="flex h-5 w-5 cursor-pointer items-center justify-center rounded-full bg-[#3B3B3B] text-white transition-all duration-200 hover:scale-110 2xl:h-6 2xl:w-6 dark:hover:opacity-80">
            ...
            try {
              ...
              return popularity.max > 0 || ad?.impression > 0 ? (
                <div className="relative inline-flex">
                  <div className="relative inline-flex">
                    <button className="peer flex h-[22px] w-[22px] cursor-pointer items-center justify-center rounded-full bg-[#3B3B3B] text-white transition-all duration-200 hover:scale-110 2xl:h-6 2xl:w-6 dark:hover:opacity-80">
                      <img
                        src={MoreOptionsIcon}
                    ...
                    <div className="absolute top-[100%] left-1/2 z-50 hidden -translate-x-1/2 opacity-0 peer-hover:flex peer-hover:opacity-100 hover:flex hover:opacity-100">
                      <div className="mt-1.5 flex flex-col overflow-hidden rounded-lg border border-white/10 bg-[#0D0D0D]/50 p-0 shadow-lg backdrop-blur-[50px] transition-all duration-150">
                        {/* Trends */}
                        <button className="backdrop-blur-80 text-10 flex items-center justify-start gap-1.5 p-1.5 px-3 whitespace-nowrap text-[#C2C0C0] hover:bg-[#0D0D0D]/50 hover:text-white 2xl:p-2 2xl:text-xs">
                          <TrendingUp className="h-3 w-3 2xl:h-4 2xl:w-4" />
                          Popularity : {popularity.max ?? 'N/A'} %
                        </button>

                        <div className="mx-auto h-[0.2px] w-full max-w-[80%] bg-white/20"></div>

                        {/* Impression */}
                        {ad?.impression > 0 && (
                          <button className="backdrop-blur-80 text-10 flex items-center justify-start gap-1.5 p-1.5 px-3 pr-7 whitespace-nowrap text-[#C2C0C0] hover:bg-[#0D0D0D]/50 hover:text-white 2xl:p-2 2xl:pr-6 2xl:text-xs">
                            <img
                              src={FlameIcon}
                              alt="Flame"
                              className="relative left-[1px] mr-1 h-3 w-3 object-contain 2xl:h-4 2xl:w-4"
                            />
          ...
          {['facebook', 'instagram', 'pinterest'].includes(ad?.network) && (
            <ShadcnTooltip label={'Ad Preview'}>
              <button
                ...
                className="flex h-5 w-5 cursor-pointer items-center justify-center rounded-full bg-[#3B3B3B] text-white transition-all duration-200 hover:scale-110 2xl:h-6 2xl:w-6 dark:hover:opacity-80"
              >
                <FaPlay className="relative h-2 w-2 2xl:left-[1px] 2xl:h-2.5 2xl:w-2.5" />
              </button>
            </ShadcnTooltip>
          ...
          <ShadcnTooltip label={networkLabels[ad?.network]}>
            <button className="flex cursor-pointer items-center justify-center gap-1 rounded-full text-center text-white/70 transition-all duration-200 hover:scale-110 dark:hover:opacity-80">
              {networks[ad?.network]}
            </button>
          </ShadcnTooltip>
          ...
          <ShadcnTooltip label="Recreate successful ad">
            <button
              className="flex h-6 w-6 cursor-pointer items-center justify-center rounded-full bg-[#3B3B3B] p-0.5 transition-all duration-200 hover:scale-110 2xl:h-7 2xl:w-7"
              onClick={(e) => {
                if (isAdLibraryRoute) {
                  e.stopPropagation();
