...
const SavedCreatives = ({ onOpenCreative, onDeleteCreative }) => {
  ...
  if (!postnodecreatives?.length) {
    return (
      <div className="-mt-16 flex w-full flex-col items-center justify-center text-white/60">
  ...
  return (
    <div className="h-full w-full overflow-auto p-6 pt-20 md:pt-8 lg:px-8 lg:py-10">
      <h2 className="mb-6 text-xl font-semibold text-white">Saved Creatives</h2>
      ...
      <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5 2xl:gap-5">
        {postnodecreatives.map((creative) => (
          <div
            ...
            className="cursor-pointer overflow-hidden rounded-2xl border border-white/20 bg-[#141414] transition-all duration-300"
            onClick={() => onOpenCreative(creative.creativeId, true)}
          >
            {/* IMAGE */}
            <div className="group relative h-45 w-full cursor-pointer overflow-hidden rounded-t-2xl bg-black/20 2xl:h-60">
              <img
            ...
            {/* FOOTER */}
            <div className="flex flex-col gap-2 bg-[#0f0f0f] p-3 2xl:p-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  {/* Placeholder logo */}
                  <div className="flex h-5.5 w-5.5 items-center justify-center rounded-full bg-[#2a2a2a] text-xs font-bold text-black 2xl:h-8 2xl:w-8 2xl:text-base">
                    ⚡
                  </div>
                  <span className="text-sm font-medium text-white 2xl:text-lg">
                    {creative.headline || 'AdsGPT'}
                  </span>
                </div>

                <span className="text-[11px] text-white/50 2xl:text-xs">
              ...
              <a
                ...
                className="mt-1 w-fit rounded-full bg-white px-4 py-1.5 text-[11px] font-medium text-[#0f0f0f] transition hover:bg-white/80 2xl:mt-2 2xl:text-sm"
              >
