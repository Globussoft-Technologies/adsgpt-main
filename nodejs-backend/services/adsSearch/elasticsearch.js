...
const client = require("../../db/elasticsearchAds");
const axios = require('axios')
const { processExploreAdsAdata , processExploreAdsAdataPAS, processCommonAdsData} = require("./adsHandler");
const logger = require("../../utils/logger");

const INDEX_NAME = process.env.INDEX_NAME;
const GATEWAY_BASE_URL = process.env.GATEWAY_URL || process.env.SOCKET_URL;
...
if((platform || platform.length>0 || platform.length == 0 )|| (competitor && competitor.length>0)){
  ...
  const urls = [
    `${GATEWAY_BASE_URL}/adsgpt/chat/get-chat-message/${uid}`,
    `${GATEWAY_BASE_URL}/adsgpt/adCreative/get-creativechat-message/${uid}`,
    `${GATEWAY_BASE_URL}/adsgpt/adCopy/get-copychat-message/${uid}`
  ];
...
const advertiserSearch = async (advertiserName) => {
  try {
      if (!GATEWAY_BASE_URL) {
          throw new Error("GATEWAY_URL or SOCKET_URL is not defined in environment variables.");
      }
